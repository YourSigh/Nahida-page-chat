...
(() => {
  ...
  // 重要：content script 运行在隔离世界，直接替换 window.fetch / XHR 未必能影响页面脚本。
  // 这里改为“注入到页面上下文 + postMessage 回传”，并在工具侧做 performance 兜底。
  ...
  const ensureApiHooked = () => {
    ...
    try {
      ...
      // 如果注入失败，就只靠 performance 兜底（工具里会返回空也没关系）。
    }
  };
  ...
  const tool_get_api_endpoints = async ({
    ...
    // 兜底：利用 PerformanceResourceTiming 里的 resource 记录（initiatorType 通常包含 fetch/xmlhttprequest）。
    const perfItems = [];
    try {
    ...
    const slice = all.slice(Math.max(0, all.length - Math.max(1, Math.min(2000, maxEntries))));
    ...
    for (const item of slice) {
      ...
      if (!map.has(key)) {
        map.set(key, {
          ...
          url: item?.url || "",
          kinds: new Set([item?.kind || "unknown"]),
          count: 1,
          lastTs: item?.ts || 0
        });
      } else {
    ...
    const endpoints = Array.from(map.values()).map((v) => ({
      method: v.method,
      url: v.url,
      count: v.count,
  ...
  const tool_get_api_responses = async ({
    ...
    // 开启响应抓取，等待一小段时间让页面发出的请求返回。
    setCaptureCfg({
      enabled: true,
      captureResponses: true,
      maxResponses: Math.max(1, Math.min(200, Number(maxEntries) || 20)),
      maxResponseChars: Math.max(256, Math.min(20000, Number(maxResponseChars) || 4000))
    });
